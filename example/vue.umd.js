var Vue = (function (exports) {
  ...
  function createGetter(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
      if (key === '__v_isReactive' /* ReactiveFlags.IS_REACTIVE */) {
        ...
        key === '__v_isShallow' /* ReactiveFlags.IS_SHALLOW */
      ) {
        return shallow;
      } else if (
  ...
  class ComputedRefImpl {
    constructor(getter, _setter, isReadonly, isSSR) {
      ...
      this['__v_isReadonly' /* ReactiveFlags.IS_READONLY */] =
        isReadonly;
    }
    get value() {
  ...
  function normalizeSuspenseChildren(vnode) {
    ...
    const isSlotChildren =
      shapeFlag & 32; /* ShapeFlags.SLOTS_CHILDREN */
  ...
  function setTransitionHooks(vnode, hooks) {
    if (
      vnode.shapeFlag & 6 /* ShapeFlags.COMPONENT */ &&
      vnode.component
    ) {
  ...
  const onBeforeMount = createHook(
    'bm' /* LifecycleHooks.BEFORE_MOUNT */,
  );
  const onMounted = createHook('m' /* LifecycleHooks.MOUNTED */);
  const onBeforeUpdate = createHook(
    'bu' /* LifecycleHooks.BEFORE_UPDATE */,
  );
  ...
  function onErrorCaptured(hook, target = currentInstance) {
    injectHook(
      'ec' /* LifecycleHooks.ERROR_CAPTURED */,
      hook,
      target,
    );
  }
  ...
  */
  ...
  const initSlots = (instance, children) => {
    if (
      instance.vnode.shapeFlag & 32 /* ShapeFlags.SLOTS_CHILDREN */
    ) {
  ...
  function setRef(
    ...
    const refs =
      owner.refs === EMPTY_OBJ ? (owner.refs = {}) : owner.refs;
  ...
  const isComment = node =>
    node.nodeType === 8; /* DOMNodeTypes.COMMENT */
  ...
  function createBaseVNode(
    ...
    if (
      ...
      (vnode.patchFlag > 0 ||
        shapeFlag & 6) /* ShapeFlags.COMPONENT */ &&
  ...
  function isStatefulComponent(instance) {
    return (
      instance.vnode.shapeFlag & 4 /* ShapeFlags.STATEFUL_COMPONENT */
    );
  }
  ...
  function useCssModule(name = '$style') {
    /* istanbul ignore else */
    {
      {
        warn$1(
          `useCssModule() is not supported in the global build.`,
        );
      }
      return EMPTY_OBJ;
    }
  ...
  function isVSlot(p) {
    return (
      p.type === 7 /* NodeTypes.DIRECTIVE */ && p.name === 'slot'
    );
  }
  ...
  function injectProp(node, prop, context) {
    ...
    if (props == null || isString(props)) {
      ...
      props.type === 15 /* NodeTypes.JS_OBJECT_EXPRESSION */
    ) {
  ...
  function parseChildren(context, mode, ancestors) {
    ...
    if (
      ...
      const shouldCondense =
        context.options.whitespace !== 'preserve';
  ...
  function parseComment(context) {
    ...
    if (!match) {
      ...
      advanceBy(
        context,
        match.index + match[0].length - prevIndex + 1,
      );
    }
    return {
  ...
  function parseElement(context, ancestors) {
    ...
    if (
      ...
      context.options.isVoidTag(element.tag)
    ) {
  ...
  function parseInterpolation(context, mode) {
    ...
    const preTrimContent = parseTextData(
      context,
      rawContentLength,
      mode,
    );
    ...
    switch (node.type) {
      case 1 /* NodeTypes.ELEMENT */:
        ...
        if (!flag) {
          ...
          if (
            generatedPropsType === 0 /* ConstantTypes.NOT_CONSTANT */
          ) {
            constantCache.set(
              node,
              0 /* ConstantTypes.NOT_CONSTANT */,
  ...
  function genVNodeCall(node, context) {
    ...
    if (isBlock) {
      push(
        `(${helper(OPEN_BLOCK)}(${disableTracking ? `true` : ``}), `,
      );
    }
    if (pure) {
      push(PURE_ANNOTATION);
  ...
  function getParentCondition(node) {
    while (true) {
      if (
        node.type === 19 /* NodeTypes.JS_CONDITIONAL_EXPRESSION */
      ) {
        if (
          node.alternate.type ===
        ...
        node.type === 20 /* NodeTypes.JS_CACHE_EXPRESSION */
      ) {
        node = node.value;
      }
    }
  ...
  function parseForExpression(input, context) {
    ...
    if (iteratorMatch) {
      ...
      if (keyContent) {
        ...
        result.key = createAliasExpression(
          loc,
          keyContent,
          keyOffset,
        );
  ...
  function mergeAsArray$1(existing, incoming) {
    if (
      existing.value.type === 17 /* NodeTypes.JS_ARRAY_EXPRESSION */
    ) {
  ...
  const transformSlotOutlet = (node, context) => {
    if (isSlotOutlet(node)) {
      const { children, loc } = node;
      const { slotName, slotProps } = processSlotOutlet(
        node,
        context,
      );
      const slotArgs = [
  ...
  const transformOn = (dir, node, context, augmentor) => {
    ...
    let shouldCache =
      context.cacheHandlers && !exp && !context.inVOnce;
    if (exp) {
  ...
  function decodeHtmlBrowser(raw, asAttr = false) {
    ...
    if (asAttr) {
      decoder.innerHTML = `<div foo="${raw.replace(
        /"/g,
        '&quot;',
      )}">`;
  ...
  const isEventOptionModifier = /*#__PURE__*/ makeMap(
